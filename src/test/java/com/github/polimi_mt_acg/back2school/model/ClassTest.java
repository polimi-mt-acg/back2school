...
public class ClassTest {
    ...
    public void testClassEntity() {
        List<Class> seedClasses = (List<Class>) DatabaseSeeder
                .getEntitiesListFromSeed("scenarioA_unit_tests", "classes.json");

        // get entity from database
        Class databaseEntity = DatabaseHandler
                .getInstance().getListSelectFrom(Class.class).get(0);
        ...
        assertEquals(
                seedEntity.getAcademicYear(),
                databaseEntity.getAcademicYear()
        );
        assertEquals(
                seedEntity.getName(),
        ...
        assertEquals(
                databaseStudent.getEmail(),
                seedEntity.seedStudentsEmail.get(0)
        );
    }
}
